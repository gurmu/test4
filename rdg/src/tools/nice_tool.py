"""
NICE inContact Function Tool
Wraps the NICE inContact FastAPI service as an Azure AI Agent function tool
"""

import httpx
import logging
from typing import Dict, Any

logger = logging.getLogger(__name__)


def create_nice_tool_definition() -> Dict[str, Any]:
    """
    Create the function tool definition for NICE callback queue
    This is what the agent sees as an available tool
    """
    return {
        "type": "function",
        "function": {
            "name": "create_nice_callback",
            "description": """Creates a callback queue entry in NICE inContact for customer follow-up.
            
            Use this tool to schedule a callback when:
            - Incident priority is High or Critical
            - Customer requires immediate attention
            - Issue needs human follow-up
            
            The callback will be routed to the appropriate skill queue based on the incident type.
            
            Required fields:
            - skillId: Routing skill ID (use "4354630" for technical support)
            - phoneNumber: Customer phone number
            - emailFrom: Customer email address
            
            Optional fields:
            - firstName, lastName: Customer name
            - priority: Queue priority 1-10 (8-10 for High/Critical)
            
            Returns contact ID for tracking the callback.""",
            "parameters": {
                "type": "object",
                "properties": {
                    "skillId": {
                        "type": "string",
                        "description": "Skill ID for routing. Use '4354630' for technical support, '5678901' for billing, '9988776' for general assistance",
                        "default": "4354630"
                    },
                    "mediaType": {
                        "type": "string",
                        "enum": ["chat", "voice", "email", "workitem"],
                        "description": "Communication channel",
                        "default": "chat"
                    },
                    "phoneNumber": {
                        "type": "string",
                        "description": "Customer phone number (10+ digits)"
                    },
                    "emailFrom": {
                        "type": "string",
                        "description": "Customer email address",
                        "format": "email"
                    },
                    "emailBccAddress": {
                        "type": "string",
                        "description": "BCC email address (optional, defaults to emailFrom)",
                        "format": "email"
                    },
                    "firstName": {
                        "type": "string",
                        "description": "Customer first name"
                    },
                    "lastName": {
                        "type": "string",
                        "description": "Customer last name"
                    },
                    "priority": {
                        "type": "integer",
                        "minimum": 1,
                        "maximum": 10,
                        "description": "Queue priority: 8-10 (High/Critical), 5-7 (Medium), 1-4 (Low)",
                        "default": 5
                    },
                    "notes": {
                        "type": "string",
                        "description": "Additional notes or context for the callback"
                    }
                },
                "required": ["skillId", "phoneNumber", "emailFrom"]
            }
        }
    }


class NICETool:
    """
    Function tool implementation for NICE inContact API
    Handles actual HTTP calls to the FastAPI service
    """
    
    def __init__(self, api_url: str, timeout: int = 30):
        """
        Initialize NICE tool
        
        Args:
            api_url: Base URL of NICE API (e.g., http://localhost:8001)
            timeout: Request timeout in seconds
        """
        self.api_url = api_url.rstrip("/")
        self.timeout = timeout
        self.client = httpx.AsyncClient(timeout=timeout)
        
        logger.info(f"NICE tool initialized: {self.api_url}")
    
    def _clean_phone_number(self, phone: str) -> str:
        """Clean and validate phone number"""
        cleaned = ''.join(filter(str.isdigit, phone))
        if len(cleaned) < 10:
            logger.warning(f"Invalid phone number: {phone}, using fallback")
            return "9999999999"
        return cleaned
    
    async def execute(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute the NICE callback creation
        
        Args:
            arguments: Function arguments from the agent
            
        Returns:
            Response from NICE API
        """
        logger.info(f"Creating NICE callback for: {arguments.get('emailFrom')}")
        
        try:
            # Clean phone number
            phone = self._clean_phone_number(arguments["phoneNumber"])
            
            # Prepare request payload matching FastAPI schema
            payload = {
                "skillId": arguments.get("skillId", "4354630"),
                "mediaType": arguments.get("mediaType", "chat"),
                "workItemQueueType": arguments.get("workItemQueueType"),
                "isActive": True,
                "phoneNumber": phone,
                "emailFromEditable": True,
                "emailFrom": arguments["emailFrom"],
                "emailBccAddress": arguments.get("emailBccAddress", arguments["emailFrom"]),
                "firstName": arguments.get("firstName"),
                "lastName": arguments.get("lastName"),
                "priority": arguments.get("priority", 5),
                "targetAgentId": arguments.get("targetAgentId"),
                "notes": arguments.get("notes")
            }
            
            # Call NICE API
            url = f"{self.api_url}/callback-queue"
            logger.debug(f"POST {url}")
            logger.debug(f"Payload: {payload}")
            
            response = await self.client.post(url, json=payload)
            response.raise_for_status()
            
            result = response.json()
            
            logger.info(f"✓ Callback created successfully")
            logger.info(f"  Contact ID: {result.get('contactId')}")
            logger.info(f"  Status: {result.get('data', {}).get('status')}")
            
            return {
                "success": True,
                "contactId": result.get("contactId"),
                "status": result.get("data", {}).get("status"),
                "message": result.get("message"),
                "full_response": result
            }
            
        except httpx.HTTPStatusError as e:
            error_msg = f"HTTP error {e.response.status_code}: {e.response.text}"
            logger.error(f"✗ {error_msg}")
            return {
                "success": False,
                "error": error_msg,
                "status_code": e.response.status_code
            }
            
        except httpx.RequestError as e:
            error_msg = f"Request error: {str(e)}"
            logger.error(f"✗ {error_msg}")
            return {
                "success": False,
                "error": error_msg
            }
            
        except Exception as e:
            error_msg = f"Unexpected error: {str(e)}"
            logger.error(f"✗ {error_msg}")
            return {
                "success": False,
                "error": error_msg
            }
    
    async def close(self):
        """Close HTTP client"""
        await self.client.aclose()
    
    def __del__(self):
        """Cleanup on deletion"""
        try:
            import asyncio
            asyncio.run(self.close())
        except:
            pass


# Standalone test function
async def test_nice_tool():
    """Test the NICE tool directly"""
    tool = NICETool("http://localhost:8001")
    
    test_args = {
        "skillId": "4354630",
        "phoneNumber": "+1234567890",
        "emailFrom": "test@company.com",
        "firstName": "Test",
        "lastName": "User",
        "priority": 8,
        "notes": "Test callback from function tool"
    }
    
    result = await tool.execute(test_args)
    print("Result:", result)
    
    await tool.close()


if __name__ == "__main__":
    import asyncio
    asyncio.run(test_nice_tool())